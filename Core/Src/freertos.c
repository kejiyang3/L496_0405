...
#define ECG_DEBUG_ENABLE_PPG RECORD_ENABLE_PPG
#define ECG_DEBUG_ENABLE_ICM RECORD_ENABLE_ICM
...
void StartTask_LVGL(void *argument)
{
  ...
  for(;;) {
    APP_LVGL_Process();
    osDelay(APP_LVGL_GetProcessDelayMs());
  }
...
void StartTask_Sensor(void *argument)
{
  ...
  /* I2C3 传感器一次性初始化。先初始化 ICM，避免 PPG/TXS 异常影响已验证的 IMU 基线。 */
  ...
  g_ppg_init_ret = (uint32_t)ppg_ret;
  if (ppg_ret != MAX30102_INIT_OK) {
      /* PPG 失败时重置 STM32 侧 I2C3，避免失败事务残留影响后续 ICM 运行期读数。 */
      APP_I2C3_BusRecover();
  }
#else
  ppg_ret = MAX30102_INIT_NOT_FOUND;
  ...
  for (;;) {
    ...
    if (g_ecg_rec.request_start) {
      ...
      if (g_ecg_rec.state == ECG_REC_IDLE ||
        ...
        // [DEBUG] 跳过 PPG/ICM I2C 操作以排除 I2C NACK 问题
        // {
        //     uint8_t s1, s2;
