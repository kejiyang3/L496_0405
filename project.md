...
| USB CDC | HAL | ⚠️ 不稳定 | — | 高频输出易断开 |
| BLE 通信 | `Core/Src/freertos.c:StartTask_BLE` | ❌ 未实现 | Codex | UART1 DMA 接收框架就绪，协议待开发 |
| 录音控制 | `Core/Src/ecg_record_control.c` | ✅ 正常 | Codex | 启停 + 状态机 |
| 音频 | `Core/Src/audio_recorder.c` | ✅ 正常 | Codex | SAI MIC 采集 |
...
| 时间同步 | — | ✅ 正常 | Codex | BLE UART1 软件 RTC |
...
| 优先级 | 模块 | 说明 |
|--------|------|------|
| 🔴 高 | BLE 协议 | SYNC/FNAME/PING/ECGO 命令处理 |
| 🔴 高 | LVGL UI 重构 | 三页滑动：表盘→ECG波形→设备信息 |
| 🟡 中 | USB CDC 稳定性 | 高频输出断开问题 |
| 🟡 中 | Flash 校验 | checksum mismatch 根因 |
| 🟢 低 | DMP 清理 | 移除未使用的 ~14KB DMP 固件 |
| 🟢 低 | Madgwick AHRS | 集成姿态解算 |
...
| 2026-05-30 | — | 软件 | SD 卡闭环验收通过 |
| 2026-05-28 | — | 硬件 | 换板：四模态全部正常 |
| 2026-05-28 | — | 硬件 | 更换 TF 卡座 |
